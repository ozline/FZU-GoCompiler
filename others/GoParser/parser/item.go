...
func (p *Parser) getAllSymbols() []Symbol {
	var symbols []Symbol
...
func (p *Parser) gotoState(items LR1Items, sym Symbol) LR1Items {
	...
	for _, item := range items {
		...
		if item.Position < len(item.Production.Body) && item.Production.Body[item.Position] == sym {
			newProd := item.Production
			newItem := LR1Item{
				Production: newProd,
				Position:   item.Position + 1,
				Lookahead:  item.Lookahead,
			}
			// fmt.Printf("计算符号[%v]的状态转移: [%v, %v, %v] -> [%v, %v, %v]\n", sym, item.Production.Head, item.Production.Body, item.Lookahead, newItem.Production.Head, newItem.Production.Body, newItem.Lookahead)
			...
			if !p.Grammar.IsTerminal(nextSym) {
				...
				for _, prod := range p.Grammar.Productions {
					...
					if prod.Head == nextSym {
						if len(prod.Body) == 0 {
							newItem := LR1Item{Production: prod, Position: 0, Lookahead: item.Lookahead}
							...
							for lookahead := range lookaheads {
								newItem := LR1Item{Production: prod, Position: 0, Lookahead: lookahead}

								if !contains(closure, newItem) {
									// fmt.Printf("添加新的项到闭包: [%v, %v, %v]\n", newItem.Production.Head, newItem.Production.Body, newItem.Lookahead)
									closure = append(closure, newItem)
									changed = true
								}
							}
...
func (p *Parser) computeLookahead(symbols []Symbol, lookahead Terminal) map[Terminal]bool {
	// 如果后面没有符号了，展望符就是当前项的展望符
	if len(symbols) == 0 {
		return map[Terminal]bool{lookahead: true}
	}

	allNullable := true
	firstSet := make(map[Terminal]bool)
	for _, sym := range symbols {
		if p.Grammar.IsTerminal(sym) {
			// 如果是终结符，就将这个终结符添加到firstSet中
			firstSet[Terminal(sym)] = true
			break
		}
