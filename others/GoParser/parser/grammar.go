...
// NewGrammar 初始化一个文法
func NewGrammar(rules []Production, terminals []Terminal) *Grammar {
	return &Grammar{
		Productions: rules,
		Terminals:   terminals,
	}
...
func (p *Parser) InitFirstSet() {
	...
	for _, t := range p.Grammar.Terminals {
		// 每个终结符的First集合只包含自身
		firstSet[Symbol(t)] = map[Terminal]bool{t: true}
	}
	...
	for _, p := range p.Grammar.Productions {
		if _, exists := firstSet[p.Head]; !exists {
			// 每个非终结符的First集合为空
			firstSet[p.Head] = make(map[Terminal]bool)
		}
	}
		...
		for _, p := range p.Grammar.Productions {
			...
			for _, sym := range p.Body {
				...
				if symFirstSet, isNonTerminal := firstSet[sym]; isNonTerminal {
					...
					if !headFirstSet[Terminal(sym)] {
						headFirstSet[Terminal(sym)] = true
						changed = true
					}
					break
				}
...
func (p *Parser) InitFollowSet() error {
	...
	for _, prod := range p.Grammar.Productions {
		if _, exists := followSet[prod.Head]; !exists {
			followSet[prod.Head] = make(map[Terminal]bool)
		}
	}

	// 初始化增广产生式头部的Follow集
	if _, exists := followSet[ARGUMENTED_PRODUCTION.Head]; !exists {
		followSet[ARGUMENTED_PRODUCTION.Head] = make(map[Terminal]bool)
	}
	// 将$加入到开始符号的Follow集中
	...
	for changed {
		...
		for _, prod := range p.Grammar.Productions {
			for i := 0; i < len(prod.Body); i++ {
				...
				if followSetSym == nil {
					followSetSym = make(map[Terminal]bool)
				}
...
func (p *Parser) computeFirstSetOfSymbol(sym Symbol) map[Terminal]bool {
	if p.Grammar.IsTerminal(sym) {
		return map[Terminal]bool{Terminal(sym): true}
	}
	return p.FirstSet[sym]
}

// computeFirstSetOfSequence 计算符号串的First集合
func (p *Parser) computeFirstSetOfSequence(sequence []Symbol) map[Terminal]bool {
	firstSetSeq := make(map[Terminal]bool)
...
func (p *Parser) containsEpsilon(firstSet map[Terminal]bool) bool {
