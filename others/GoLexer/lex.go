...
func (l *Lexer) NextToken() (Token, error) {
	...
	if ch == '"' {
		...
		return Token{Type: STRING, Value: sb.String()}, nil
	}
	...
	if isLetter(ch) {
		...
		return Token{Type: tokenType, Value: word}, nil
	}
	...
	if isDigit(ch) {
		...
		return Token{Type: NUMBER, Value: sb.String()}, nil
	}

	// 检查是否为运算符
	if tokenType, ok := operators[string(ch)]; ok {
		return Token{Type: tokenType, Value: string(ch)}, nil
	}
